...
If HotKeysAllow = False Then Exit Sub
    If GetKeyState(vbKeyControl) < 0 Then
        ...
        Else
            If GetKeyState(vbKeyZ) < 0 Then DePegar
...
finGrhIndex = SupData(89).Grh + 16

If MapData(X, Y).Graphic(1).GrhIndex >= SupData(89).Grh And MapData(X, Y).Graphic(1).GrhIndex <= finGrhIndex Then
